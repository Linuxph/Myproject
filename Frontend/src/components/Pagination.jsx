...
useEffect(() => {
  ...
  const fetchMovies = async () => {
    try {
      const response = await fetch("http://localhost:3000/api/v1/home?page=2", {
        method: "GET",
        headers: {
...
if(window.innerWidth > 640)
  ...
  return (
    ...
    {/* {signIn ? ( */}
      <div className=" w-[100%]  fixed p-5 flex flex-wrap justify-center gap-8 xl:justify-normal  p-5">
          ...
          movies.map((movie) => (
            ...
            >
              <motion.div className="h-[50%] xl:h-[70%] w-[100%] border-2 md:border-0 overflow-hidden rounded-3xl">
              ...
              <motion.button onClick={() => { localStorage.setItem("movieId",selectedId); navigate('/showtime') }} className="bg-pink-500 border-2 text-white font-bold rounded-2xl p-1 relative bottom-0  md:hidden">Book </motion.button>
            </motion.div>
            ...
            <div>
              ...
              >
                ...
                <div className="h-[100%]  ">
                  ...
                  <br />
                  <motion.h2 className="bg-black/40 rounded-3xl p-5 mb-5">
                    <b>Overview</b> <br />
                    {movies.find((i) => i._id === selectedId).description}
                  </motion.h2>
                  <motion.h2 className="mb-5 font-bold ">
                    Release Date :
                    {
                      movies
    ...
    {/* // ):(
    //   <div className=" h-[10vh] bg-black/40 text-center w-[50vw] rounded-3xl ">
